...
def index_docs():
    ...
    # 7. Find document files (PDF + Markdown/Text)
    ...
    if not all_files:
        logger.warning(f"No documents found in {data_folder}. Please add PDF, MD, or TXT files.")
        return
    
    logger.info(f"Found {len(all_files)} documents to process: {[os.path.basename(f) for f in all_files]}")
    ...
    for doc_path in all_files:
        try:
            logger.info(f"Loading: {os.path.basename(doc_path)}...")
