"""
Workflow Definition for the HealthGuard AI.
...
def create_upload_graph():
    """
    Constructs a graph for local file uploads (skips yt-dlp download).
    Flow: [START] -> [indexer_upload] -> [auditor] -> [END]
    """
    workflow = StateGraph(VideoAuditState)
    workflow.add_node("indexer_upload", index_uploaded_video_node)
    workflow.add_node("auditor", audit_content_node)
    workflow.set_entry_point("indexer_upload")
    workflow.add_edge("indexer_upload", "auditor")
    workflow.add_edge("auditor", END)
    return workflow.compile()


# Expose the runnable apps for import by the API or CLI
app = create_graph()
upload_app = create_upload_graph()