...
class VideoIndexerService:
    def __init__(self):
        ...
        self.vi_name = os.getenv("AZURE_VI_NAME", "project-healthguard-001")
        self.credential = DefaultAzureCredential()
        self._current_url = ""  # Track source platform
    ...
    def download_video(self, url, output_path="temp_video.mp4"):
        """Downloads a video from YouTube or TikTok to a local file."""
    ...
    def extract_data(self, vi_json):
        ...
        logger.info(f"Azure VI response keys: {list(vi_json.keys())}")
        videos = vi_json.get("videos", [])
        logger.info(f"Number of videos in response: {len(videos)}")
        if videos:
            insights_keys = list(videos[0].get("insights", {}).keys())
            logger.info(f"Insights keys: {insights_keys}")
        ...
        return {
            ...
            "video_metadata": {
                ...
                "platform": "tiktok" if "tiktok.com" in self._current_url else "youtube"
            }
        }